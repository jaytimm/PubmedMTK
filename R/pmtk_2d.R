...
#' @rdname pmtk_2d
#' 
pmtk_2d <- function(x, method = NA) {
  
  ##### sparse matrix
  matx <- tidytext::cast_sparse(data = x,
                                row = topic_id,
                                column = feature, 
                                value = beta)
  ...
  pca <- data.frame(prcomp(dist(cormat), scale = T, center = T)$x, row.names = NULL)
  pca1 <- pca[,c('PC1', 'PC2')]
  pca1$topic_id <- as.character(1:ncol(cormat))
  colnames(pca1)[1:2] <- c('X1', 'X2')
  pca1 <- pca1[, c(3, 1:2)]
  
  ##### hierarchical cluster -- for dendrogram --
  disthi <- dist(matx, method = "euclidean") 
  hc1 <- hclust(disthi, method = "ward.D") 
